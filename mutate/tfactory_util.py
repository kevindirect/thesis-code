...
def first_nonzero(ser, ret_idx=DEFAULT_RET_IDX, idx_norm=DEFAULT_IDX_NORM):
	...
	return idx if (not isinstance(idx, np.ndarray) and (isnt(idx) or idx==0)) return idx.item(0)
...
ROW_VAL_SELECTOR_MAP = {
	0: (lambda ser: ser.loc[ROW_IDX_SELECTOR_MAP.get(0)(ser)]),
	-1: (lambda ser: ser.loc[ROW_IDX_SELECTOR_MAP.get(-1)(ser)]),
	'h': (lambda ser: ser.loc[ROW_IDX_SELECTOR_MAP.get('h')(ser)]),
	'l': (lambda ser: ser.loc[ROW_IDX_SELECTOR_MAP.get('l')(ser)]),
...
def single_row(val, flt):
	...
	"""
	return {
		bool(not val): ROW_IDX_SELECTOR_MAP.get(flt, None),
		val: ROW_VAL_SELECTOR_MAP.get(flt, None)
	}.get(True)
...
def single_row_map(val, flt, map_fn):
	return compose(single_row(val, flt), SER_MAP_FN.get(map_fn))
