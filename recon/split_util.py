...
from common_util import RECON_DIR, midx_intersect, midx_split
...
""" ********** TRAIN/TEST SPLITS ********** """
def idx_three_split(*pd_idx, val_ratio=.2, test_ratio=.2, shuffle=False):
	"""
	...
	"""
	common_idx = midx_intersect(*pd_idx)
	if (shuffle):
		common_idx = sklearn.utils.shuffle(common_idx)
	train_idx, val_idx, test_idx = midx_split(common_idx, 1-(val_ratio+test_ratio), val_ratio, test_ratio)

	return train_idx, val_idx, test_idx

def get_train_test_split(feats, lab, test_ratio=.8, to_np=True, shuffle=False):
	"""
	XXX - deprecated, use idx_three_split instead
